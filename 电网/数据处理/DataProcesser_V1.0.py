...
class DataProcess:
    ...
    def process(self):
        try:
            files = [fileName for fileName in os.listdir(self.inputPath)]
            logger.info(f"即将处理：{files}")
            for fileName in files:
                logger.info(f"文件：{fileName} | 开始处理...")
                self.readCSVToJson(fileName)
                logger.info(f"文件：{fileName} | 处理结束!")
        except Exception as e:
            logger.error(f"程序发生异常，详细信息如下：")
            logger.error(e, exc_info=True, stack_info=True)


    def readCSVToJson(self, fileName:str):

        allPoint = dict()
        allLine = dict()
        with open(f"{self.inputPath}/{fileName}", encoding="utf-8") as f:
            data = f.readlines()
        ...
        with open(f"{self.outputPath}/{geoJsonFileName}.json", "w", encoding="utf-8") as f:
            json.dump(geoJson, f)
    ...
    def handleRow(self, row,allPoint, allLine, fileName):
        ...
        if not self.hasPointType:
            pointName = row[1]
            tName = row[2]
            myPoint = MyPoint("", row[1], tName, float(row[4]), float(row[5]))
        else:
            pointName = row[2]
            tName = row[3]
            myPoint = MyPoint(row[1], row[2], tName, float(row[5]), float(row[6]))
